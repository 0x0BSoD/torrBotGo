...
docker run -v "$(pwd)/config.yaml":/config.yaml zlodey23/transmission-bot
```
...
services:
  torrbot:
    image: zlodey23/transmission-bot:latest
    container_name: torrbot
...
**Note**: The `images` directory cannot be `/tmp` in Docker containers.
